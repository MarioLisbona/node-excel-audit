...
export const extractResponsesForReport = async (
  client,
  userId,
  workbookId,
  sheetName
  ...
  try {
    // Construct the URL for the Excel file's used range
    const range = `https://graph.microsoft.com/v1.0/users/${userId}/drive/items/${workbookId}/workbook/worksheets/${sheetName}/range(address='D8:D1000')`;

    // Fetch the data from column D
    const response = await client.api(range).get();

    // Extract the values from the response
    const data = response.values;

    // Filter out empty cells and flatten the array (since each cell is in its own array)
    const columnData = data
      .filter((row) => row[0] !== null && row[0] !== "")
      .map((row) => row[0]);

    console.log("Number of items found:", columnData.length);

    return columnData;
  } catch (error) {
    console.error("Error retrieving column data:", error.message);
    console.error("Full error details:", error);
    throw error;
  }
};
